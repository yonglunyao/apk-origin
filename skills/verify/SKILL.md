...
- **可选**：联网 MCP（搜索 / 网页读取 / GitHub 读取），由用户在 `~/.claude.json` 自行配置，插件不绑定具体服务商
...
PLUGIN_ROOT="<skill_base>/../.."   # skill base = 本 SKILL.md 所在目录，插件根 = 上溯两级
...
此后该 Bash 内 `jadx` / `readelf` / `nm` / `strings` / `objdump` 直接可用（Windows 下 jadx 用 jadx.bat）。
